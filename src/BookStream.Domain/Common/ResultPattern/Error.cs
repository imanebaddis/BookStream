...
namespace BookStream.Domain.Common.ResultPattern
{
    public class Error
{
    private Error(
    ...
    {
        return new(code, description, ErrorType.AccessUnAuthorized);
    }
    public static Error AccessForbidden(string code, string description)
    {
        return new(code, description, ErrorType.AccessForbidden);
    }
}
}